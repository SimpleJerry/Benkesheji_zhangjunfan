...
{
    public partial class 历史订单查询_买Form : Form
    {
        private string U_account;
        private int U_id;
        public 历史订单查询_买Form(string U_account)
        {
        ...
        {
            if (查询textBox.Text == "")
                SqlHelper.ShowDataWithDataGirdView(dataGridView1, string.Format("SELECT 订单号,货物编号,卖家UID,卖家昵称,价格,数量,交易时间,收货地址,快递单号,订单状态 FROM 历史订单视图 WHERE 买家UID = {0}; ", U_id));
            else
                SqlHelper.ShowDataWithDataGirdView(dataGridView1, string.Format("SELECT * FROM (SELECT 订单号,货物编号,卖家UID,卖家昵称,价格,数量,交易时间,收货地址,快递单号,订单状态 FROM 历史订单视图 WHERE 买家UID = {0}) x WHERE 订单号 LIKE '%{1}%' OR 货物编号 LIKE '%{1}%' OR 订单状态 LIKE '%{1}%'; ", U_id, 查询textBox.Text));
        ...
        private void 历史订单查询_买Form_KeyPress(object sender, KeyPressEventArgs e)
        {
