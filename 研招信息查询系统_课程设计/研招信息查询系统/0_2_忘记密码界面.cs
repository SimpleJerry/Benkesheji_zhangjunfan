...
namespace 研招信息查询系统
{
    public partial class 忘记密码界面Form : Form
    {
        public 忘记密码界面Form()
        {
            InitializeComponent();
        }
        private void 忘记密码界面Form_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        {
            int id = SqlHelper.GetID("用户表", "UID");//得到一个新的不重复的编号
            string account = 帐号textBox.Text;
            string password = 密码textBox.Text;
            string password2 = 再次输入密码textBox.Text;
            string phone = 手机textBox.Text;
            string email = 邮箱textBox.Text;
            ...
            bool isphone = IS_phone(phone); //判断手机号字符串是否为11位整数字符串
            bool isemail = IS_email(email);//判断邮箱格式是否正确
            ...
            {
                if ((password == password2) && isphone && isemail)
                {
                    try
                    {
                        string UID = SqlHelper.InquireItem("用户表", "UID", String.Format("帐号='{0}' AND 手机='{1}' AND 邮箱='{2}'", account, phone, email));
                        SqlHelper.UpdateItem("用户表", string.Format("密码='{0}'", password), string.Format("UID='{0}'", UID));
                        MessageBox.Show("已成功找回！");
                        this.Close();
                    }
                    catch
                    {
                        MessageBox.Show("帐号、手机、邮箱输入错误！");
                    }
                }
                else
