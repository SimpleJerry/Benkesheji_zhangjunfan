...
namespace 研招信息查询系统
{
    public partial class 注册界面Form : Form
    {
        ...
        private void 注册button_Click(object sender, EventArgs e)
        {
            ...
            string name = 昵称textBox.Text;
            string phone = 手机textBox.Text;
            string email = 邮箱textBox.Text;
            string profession = 专业textBox.Text;
            ...
            bool isnum = IS_phone(phone); //判断手机号字符串是否为11为整数字符串
            ...
            {
                if ((password == password2) && isnum && isemail)
                {
                    try
                    {
                        SqlHelper.InsertItem("用户表(UID,帐号,密码,昵称,专业,手机,邮箱)", String.Format("('{0}','{1}','{2}','{3}','{4}','{5}','{6}');", id, account, password, name, profession, phone, email));
